package com.example.mysalesforcetempapplication.viewmodel
...
//        UUID.fromString("6ca78ac2-3393-11ef-9088-0242ac110002")
    ...
    fun resetMessagingConfig() {
        ...
        val coreConfig = CoreConfiguration
            .fromFile(getApplication(), "config.json", isUserVerificationEnabled)
    ...
    private fun registerHiddenPreChatValuesProvider(config: UIConfiguration) {
        coreClient(config).registerHiddenPreChatValuesProvider(object : PreChatValuesProvider {
            ...
            override suspend fun setValues(input: List<PreChatField>): List<PreChatField> {
                logger.log(Level.INFO,"Input Values -> $input")
                return input.onEach {
                    when (it.name) {
                        "Unique_Id" -> it.userInput = "6ca78ac2-3393-11ef-9088-0242ac110002"
